        ...
        <div className="glow-card p-8 relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-rose-500/10 border border-rose-500/20">
                    ...
                    <span className="text-[8px] font-black text-rose-400 uppercase tracking-wider">Training</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="metric-cell">
                    ...
                    <div className="text-xl font-black text-orange-400 tracking-tighter font-mono">{stats.epoch.toString().padStart(3, '0')}</div>
                </div>
                <div className="metric-cell">
                    <div className="text-[7px] font-mono text-slate-500 uppercase mb-1 flex items-center gap-1">
                        <TrendingDown size={8} className="text-rose-500" /> Loss
                    </div>
            ...
            <div className="mb-6 p-3 rounded-xl bg-black/30 border border-white/5">
                ...
                <div className="flex items-end gap-[2px] h-12">
                    {lossHistory.map((loss, i) => (
                        <div
                            key={i}
                            className="flex-1 bg-orange-500/30 rounded-t-sm min-h-[2px] transition-all duration-300"
                            style={{
            ...
            <div className="space-y-4 font-mono">
                <div className="space-y-2">
                    <div className="flex justify-between text-[9px] uppercase">
                        <span className="text-slate-500 flex items-center gap-1"><TrendingUp size={9} className="text-orange-400" /> Train_Accuracy</span>
                        <span className="text-orange-400 font-bold">{(stats.acc * 100).toFixed(2)}%</span>
                    </div>
                ...
                <div className="space-y-2">
                    <div className="flex justify-between text-[9px] uppercase">
                        <span className="text-slate-500 flex items-center gap-1"><TrendingUp size={9} className="text-rose-400" /> Val_Accuracy</span>
                        <span className="text-rose-400/80 font-bold">{(stats.val_acc * 100).toFixed(2)}%</span>
                    </div>
                    <div className="progress-bar">
                        <motion.div
                            animate={{ width: `${stats.val_acc * 100}%` }}
                            transition={{ duration: 0.8 }}
                            className="h-full rounded-full bg-gradient-to-r from-rose-600 to-rose-400 shadow-[0_0_10px_rgba(52,211,153,0.3)]"
                        />
                    </div>
                </div>
            </div>
                ...
                <div className="flex gap-[3px] items-end h-6">
                    {[...Array(24)].map((_, i) => (
                        <motion.div
                            ...
                            className="w-[3px] bg-orange-500/20 rounded-full"
                        />
                    ))}
                </div>
                <div className="text-[8px] font-mono text-slate-600 uppercase tracking-widest flex items-center gap-2">
                    <Activity size={10} className="text-orange-500/50" /> Latency: 0.8ms
                </div>
            </div>
        </div>
    );
